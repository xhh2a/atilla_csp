...
def load_data(request_parameters: RequestParameter):
    ...
    building_chains = _load_building_chains(request_parameters)

def _load_data_module(request_parameters: RequestParameter):
    data_module = import_module(f"data.{get_context(ContextVariableKeys.GAME)}.{request_parameters.culture.value}")
...
def _load_building_chains(request_parameters: RequestParameter) -> list[Building]:
    ...
    return [
        ...
        *(request_parameters.secondary_religion.building_chains if request_parameters.secondary_religion else [])
    ]
